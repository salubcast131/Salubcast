...
.feed-card-media {{ width:100%; height:190px; border-radius:18px; overflow:hidden; position:relative; z-index:1; background:rgba(255,255,255,.06); }}
...
.weather-page {{ width:100%; height:100%; display:grid; place-items:center; padding:clamp(28px,4vw,56px); position:relative; overflow:hidden; background:
linear-gradient(135deg, rgba(56,189,248,.22), transparent 32%),
radial-gradient(circle at top right, rgba(125,211,252,.22), transparent 30%),
linear-gradient(180deg, #041019, #092233 46%, #0e3a45 100%); }}
.weather-page::before {{ content:''; position:absolute; inset:0; pointer-events:none; background:radial-gradient(circle at 20% 12%, rgba(255,255,255,.1), transparent 26%); }}
.weather-card {{ position:relative; z-index:1; width:min(100%, 900px); display:grid; gap:28px; text-align:center; justify-items:center; }}
.weather-card .weather-page-kicker {{ font-size:16px; letter-spacing:.32em; text-transform:uppercase; color:#bae6fd; font-weight:900; }}
.weather-card .weather-page-icon {{ width:180px; height:180px; border-radius:44px; background:linear-gradient(180deg, rgba(255,255,255,.14), rgba(255,255,255,.04)); border:1px solid rgba(255,255,255,.16); display:grid; place-items:center; position:relative; box-shadow:0 30px 60px rgba(0,0,0,.3), inset 0 1px 0 rgba(255,255,255,.08); }}
.weather-card .weather-page-icon.sun::before {{ content:''; width:76px; height:76px; border-radius:50%; background:radial-gradient(circle, #fde68a, #f59e0b 72%); box-shadow:0 0 0 22px rgba(250,204,21,.12), 0 0 60px rgba(250,204,21,.35); }}
.weather-card .weather-page-icon.cloud::before {{ content:''; position:absolute; width:96px; height:50px; border-radius:999px; background:#dbeafe; bottom:56px; box-shadow:-36px -18px 0 4px #dbeafe, 32px -22px 0 0 #dbeafe; }}
.weather-card .weather-page-icon.rain::before {{ content:''; position:absolute; width:96px; height:50px; border-radius:999px; background:#dbeafe; top:44px; box-shadow:-36px -18px 0 4px #dbeafe, 32px -22px 0 0 #dbeafe; }}
.weather-card .weather-page-icon.rain::after {{ content:''; position:absolute; width:78px; height:64px; bottom:28px; left:56px; background:
...
.weather-card .weather-page-icon.partly::before {{ content:''; position:absolute; width:68px; height:68px; border-radius:50%; background:radial-gradient(circle, #fde68a, #f59e0b 72%); top:40px; left:34px; box-shadow:0 0 0 18px rgba(250,204,21,.12); }}
.weather-card .weather-page-icon.partly::after {{ content:''; position:absolute; width:90px; height:44px; border-radius:999px; background:#e2e8f0; bottom:50px; right:32px; box-shadow:-34px -18px 0 4px #e2e8f0, 26px -20px 0 0 #e2e8f0; }}
.weather-card .weather-page-temp {{ font-size:clamp(96px, 12vw, 168px); font-weight:900; line-height:.86; letter-spacing:-.06em; }}
.weather-card .weather-page-city {{ font-size:clamp(32px, 3.4vw, 48px); font-weight:900; letter-spacing:-.03em; }}
.weather-card .weather-page-cond {{ font-size:clamp(20px, 2vw, 28px); color:#dbeafe; }}
.weather-card .weather-page-stats {{ display:flex; gap:36px; flex-wrap:wrap; justify-content:center; }}
.weather-card .weather-page-stat {{ display:grid; gap:4px; }}
.weather-card .weather-page-stat-label {{ font-size:13px; letter-spacing:.16em; text-transform:uppercase; color:#93c5fd; font-weight:800; }}
.weather-card .weather-page-stat-value {{ font-size:26px; font-weight:900; }}
.weather-card .weather-page-time {{ font-size:20px; color:#bfdbfe; font-weight:700; }}
body.portrait .weather-card .weather-page-temp {{ font-size:clamp(80px, 18vw, 140px); }}
